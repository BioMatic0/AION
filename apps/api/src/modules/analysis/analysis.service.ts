  ...
  async onModuleInit() {
    if (!this.prisma) {
      if (this.desktopState?.isEnabled()) {
        this.reports = await this.desktopState.loadSection("analysis.reports", this.reports);
        this.quantumReports = await this.desktopState.loadSection("analysis.quantumReports", this.quantumReports);
      }
      return;
    }
    ...
    if (!this.prisma) {
      if (this.desktopState?.isEnabled()) {
        if (reportType === "analysis") {
          await this.desktopState.saveSection("analysis.reports", this.reports);
        } else if (reportType === "quantum") {
          await this.desktopState.saveSection("analysis.quantumReports", this.quantumReports);
        }
      }
      return;
    }
