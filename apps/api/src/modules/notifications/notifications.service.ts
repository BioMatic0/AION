...
export class NotificationsService implements OnModuleInit {
  ...
  async onModuleInit() {
    if (!this.prisma) {
      if (this.desktopState?.isEnabled()) {
        this.preferences = await this.desktopState.loadSection("notifications.preferences", this.preferences);
        this.history = await this.desktopState.loadSection("notifications.history", this.history);
        this.jobs = await this.desktopState.loadSection("notifications.jobs", this.jobs);
      }
      return;
    }
  ...
  private async syncJobs() {
    ...
    if (!this.prisma) {
      if (this.desktopState?.isEnabled()) {
        await this.desktopState.saveSection("notifications.jobs", this.jobs);
      }
      return;
    }
  ...
  private async persistPreferences() {
    if (!this.prisma) {
      if (this.desktopState?.isEnabled()) {
        await this.desktopState.saveSection("notifications.preferences", this.preferences);
      }
      return;
    }
  ...
  private async persistHistoryItem(item: NotificationHistoryItem) {
    if (!this.prisma) {
      if (this.desktopState?.isEnabled()) {
        await this.desktopState.saveSection("notifications.history", this.history);
      }
      return;
    }
